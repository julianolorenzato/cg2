...
uniform float u_shadowSoftness;      // new uniform for shadow softness

out vec4 outColor;

float sampleShadow(vec2 offset, vec3 projectedTexcoord, float currentDepth) {
  vec2 texCoord = projectedTexcoord.xy + offset;
  float shadowDepth = texture(u_projectedTexture, texCoord).r;
  return currentDepth < shadowDepth + u_bias ? 1.0 : 0.0;
}
